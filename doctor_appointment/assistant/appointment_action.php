...
if (isset($_POST["action"])) {
	if ($_POST["action"] == 'fetch') {
		...
		if ($_SESSION['type'] == 'Assistant') {
			$order_column = array('appointments.date ASC', 'appointments.appointment_number', 'patient_database.patient_first_name', 'doctor_database.doctor_name', 'appointments.time', 'appointments.services', 'appointments.status');
			$main_query = "
			...
			$order_column = array('appointments.date ASC', 'appointments.appointment_number', 'patient_database.patient_first_name', 'appointments.time', 'appointments.services', 'appointments.status');
			...
			if ($_POST["is_date_search"] == "yes") {
				$search_query .= 'AND doctor_schedule_table.date BETWEEN "' . $_POST["start_date"] . '" AND "' . $_POST["end_date"] . '" ';
			} else {
				$search_query .= '';
			}
		...
		if (isset($_POST["order"])) {
			...
			$order_query = 'ORDER BY appointments.date ASC, appointments.appointment_id DESC ';
		}
	...
	if ($_POST["action"] == 'fetch_single') {
		...
		foreach ($appointment_data as $appointment_row) {
			...
			';
			foreach ($appointments_data as $appointments_row) {
				$html .= '
				<tr>
					<th width="40%" class="text-right">Doctor Name</th>
					<td>' . $appointments_row["doctor_name"] . '</td>
				</tr>
				<tr>
					<th width="40%" class="text-right">Appointment Date</th>
					<td>' . $appointments_row["date"] . '</td>
				</tr>
				<tr>
					<th width="40%" class="text-right">Appointment Day</th>
					<td>' . $appointments_row["doctor_schedule_day"] . '</td>
				</tr>
